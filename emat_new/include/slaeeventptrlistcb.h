///////////////////////////////////////////////////////////////////////////////
/*! \file   slaeeventptrlistcb.h
*  \author Michael Schlemper
*  \date   08.09.2006
*  \brief  Headerfile for class SlAeEventPtrListCB
*
...
#if !defined SLAEEVENTPTRLISTCB_H_
#define SLAEEVENTPTRLISTCB_H_
...
class SlAeEventPtrListCBImpl;


/*! \class SlAeEventPtrListCB
*  \brief ...
*/
class SL_AE_SVC_ADAP_EXPORTS SlAeEventPtrListCB
{
...
public:
	...
	SlAeEventPtrListCB(void);

	virtual ~SlAeEventPtrListCB(void);

	/*! \fn long init(const QString& rszLanguage, quint32 ulMaxSize = 10);
	*
	*  Initialization - must be called before using the adapter
	*/
	long init(const QString& rszLanguage, quint32 ulMaxSize = 10);

	/*! \fn long fini(qint32 lDummy = 0);
	*
	...
	*/
	...
	/*! \fn virtual void listChangedCB(const quint32 ulNrOfNewEvents) = 0;
	*
	*   \param  ulNrOfNewEvents  Number of new arrived (prepend) events
	*
	*   Signals a change of the event list. The list is working with the FIFO principle:
	...
	*   If more events arrive at the same time, than the actual size of the list can contain,
	*   succeeding callbacks are created.
	*/
	virtual void listChangedCB(const quint32 ulNrOfNewEvents) = 0;
...
private:
	...
	SlAeEventPtrListCB(const SlAeEventPtrListCB& other);
	SlAeEventPtrListCB& operator=(const SlAeEventPtrListCB& other);

	SlAeEventPtrListCBImpl* m_pImpl;
};
