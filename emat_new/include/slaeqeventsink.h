///////////////////////////////////////////////////////////////////////////////
/*! \file   slaeqeventsink.h
 *  \author Michael Schlemper
 *  \date   17.11.2004
 *  \brief  Headerfile for class SlAeQEventSink
 *
...
#if !defined SLAEQEVENTSINK_H_
#define SLAEQEVENTSINK_H_
...
public:
    ...
    SlAeQEventSink(QObject* pParent = 0, const char* pName = 0);

    virtual ~SlAeQEventSink();

    /*! \fn long init();
     *
     *  Initialization - must be called before using the adapter
    */
    long init(const QString& rszLanguage = NULL);
...
signals:
    ...
    /*! \fn void newEvent(const QList<SlAeEvent*>& rEventList);
     *
     *  Notification signal - emitted when e new event occurs
    */
    void newEvent(const QList<SlAeEvent*>& rEventList);
...
private slots:
    ...
    virtual void onNewEvent(const QList<SlAeEvent*>& rEventList);


private:

    // Hide copy possibilities
    SlAeQEventSink(const SlAeQEventSink& other);
    SlAeQEventSink& operator=(const SlAeQEventSink& other);

    SlAeQEventSinkImpl *m_pImpl;
};
