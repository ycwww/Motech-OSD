///////////////////////////////////////////////////////////////////////////////
/*! \file   slaeqeventptrlist.h
 *  \author Michael Schlemper
 *  \date   17.11.2004
 *  \brief  Headerfile for class SlAeQEventPtrList
 *
...
#if !defined SLAEQEVENTPTRLIST_H_
#define SLAEQEVENTPTRLIST_H_
...
class SlAeQEventPtrListImpl;


/*! \class SlAeQEventPtrList
 *  \brief Maintains list of all alarm events
*/
class SL_AE_SVC_ADAP_EXPORTS SlAeQEventPtrList :
    public QObject
{
    Q_OBJECT
...
public:
    ...
    SlAeQEventPtrList(QObject* pParent = 0, const char* pName = 0);

    virtual ~SlAeQEventPtrList();

    /*! \fn long init();
     *
     *  Initialization - must be called before using the adapter
    */
    long init(quint32 ulMaxSize = 10);
    ...
    */
    ...
    /*! \fn long setMaxSize(quint32 nMaxSize);
    *
    *  Set the maximum number of events to track
    */
    long setMaxSize(quint32 ulMaxSize);
...
signals:
    ...
    /*! \fn void listChanged(const quint32 ulNrOfNewEvents);
     *
     *  \param  ulNrOfNewEvents  Number of new arrived (prepend) events
     *
     *  Notification signal - Signals a change of the event list.
     *  The list is working with the FIFO principle:
     *  New events are prepend at the beginning of the list, if the list is overloaded,
     *  the oldest events are removed from the end of the list.
     *  If more events arrive at the same time, as the actual size of the list can contain,
     *  than signals are created.
    */
    void listChanged(const quint32 ulNrOfNewEvents);
...
private slots:
    ...
    virtual void onListChanged(const quint32 ulNrOfNewEvents);


private:

    // Hide copy possibilities
    SlAeQEventPtrList(const SlAeQEventPtrList& other);
    SlAeQEventPtrList& operator=(const SlAeQEventPtrList& other);

    SlAeQEventPtrListImpl *m_pImpl;
};



#endif // #if !defined SLAEQEVENTPTRLIST_H_
