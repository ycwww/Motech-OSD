...
int laserCompensate::measurement_data_comparison(const POINT_WITH_NORMAL& curPos, std::vector<SENSOR_DATA_WITH_POS>& sensorDataWithPos, int index)
{
	...
	for (int i = index; i < sensorDataWithPos.size() - 1; i++)
	{
		...
		if (/*3.0 > abs(curPos.data[3] - sensorDataWithPos.at(i).wcs[3]) &&*/
			0.5 > abs(curPos.data[4] - sensorDataWithPos.at(i).wcs[4]) &&
		...
		{
			////找到目标区域后再搜索10个点位
			//for (int j = 0; j + i < sensorDataWithPos.size() - 1 && j <= 10; j++) 
			//{
			//	abs_difference[0] = sqrt(pow(curPos.data[0] - sensorDataWithPos.at(i).wcs[0], 2) +
			//		pow(curPos.data[1] - sensorDataWithPos.at(j+i).wcs[1], 2)
			//		+ pow(curPos.data[2] - sensorDataWithPos.at(j+i).wcs[2], 2));
			//	if (/*3.0 > abs(curPos.data[3] - sensorDataWithPos.at(i).wcs[3]) &&*/
			//		0.5 > abs(curPos.data[4] - sensorDataWithPos.at(j+i).wcs[4]) &&
			//		minAbsDiff > abs_difference[0])
			//	{
			//		minAbsDiff = abs_difference[0];
			//		minIndex = j+i;
...
bool index_compare(IndexedDistance a, IndexedDistance b)
{
	return a.distance < b.distance;
}

int laserCompensate::findClosetPoints(const POINT_WITH_NORMAL& curPos,
	std::vector<SENSOR_DATA_WITH_POS>& sensorDataWithPos,
	int index,
	std::vector<int>& closestIndices)
{
	float radius = 0.0;
	float radian_difference = 0.0, distance = 0.0;

	std::vector<IndexedDistance> distances;

	for (int i = index; i < sensorDataWithPos.size() - 1; i++) {
		radius = sqrt(pow(curPos.data[1] - sensorDataWithPos.at(i).wcs[1], 2) +
			pow(curPos.data[2] - sensorDataWithPos.at(i).wcs[2], 2));
		distance = sqrt(pow(curPos.data[0] - sensorDataWithPos.at(i).wcs[0], 2) +
			pow(curPos.data[1] - sensorDataWithPos.at(i).wcs[1], 2) +
			pow(curPos.data[2] - sensorDataWithPos.at(i).wcs[2], 2));
		radian_difference = abs(curPos.data[4] - sensorDataWithPos.at(i).wcs[4]) / 180.0 * 3.1415926 * radius;
		
		distances.push_back({ i, sqrt(pow(distance, 2) + pow(radian_difference,2))});
		
	}

	// 按距离排序
	std::sort(distances.begin(), distances.end(),index_compare);

	// 获取最近的 5 个点的索引
	closestIndices.clear();
	for (size_t i = 0; i < std::min(distances.size(), size_t(5)); ++i) {
		if (50 > distances[i].distance)
		{
			closestIndices.push_back(distances[i].index);
		}
		
	}

	return closestIndices.size();
}
// 定义一个结构体用于存储满足条件的数据及其索引和距离
struct ValidData {
	SENSOR_DATA_WITH_POS data;
	int index;
	float abs_difference;
};

int laserCompensate::laser_data_comparison(const POINT_WITH_NORMAL& curPos, std::vector<SENSOR_DATA_WITH_POS>& sensorDataWithPos, int index)
{
	std::vector<ValidData> vecSensorData;
	...
	for (int i = index; i < sensorDataWithPos.size(); i++)
	{
		if (1.5 < abs(curPos.data[4] - sensorDataWithPos[i].wcs[4]))
			continue;
		if (sensorDataWithPos[i].wcs[4] - curPos.data[4] > 1.5)
			break;
		float abs_difference = std::sqrt(std::pow(curPos.data[0] - sensorDataWithPos[i].wcs[0], 2) +
			std::pow(curPos.data[1] - sensorDataWithPos[i].wcs[1], 2) +
			std::pow(curPos.data[2] - sensorDataWithPos[i].wcs[2], 2));

		if (5.0 > abs_difference)
		{
			ValidData validData;
			validData.data = sensorDataWithPos[i];
			validData.index = i;
			validData.abs_difference = abs_difference;
			vecSensorData.push_back(validData);
		}
	}

	// 按照 abs_difference 从小到大排序
	std::sort(vecSensorData.begin(), vecSensorData.end(), [](const ValidData& a, const ValidData& b) {
		return a.abs_difference < b.abs_difference;
	});

	if (!vecSensorData.empty())
	{
		float sum = 0;
		for (const auto& validData : vecSensorData)
		{
			sum += validData.data.sensorVal;
		}
		float average = sum / vecSensorData.size();
		vecSensorData[0].data.sensorVal = average;
		return vecSensorData[0].index;
	}
...
std::vector<TOOL_POINT_DIFF> laserCompensate::cubicBSplineSmoothZ(const std::vector<TOOL_POINT_DIFF>& points, double tension) {
	...
	for (size_t i = 1; i < n - 1; ++i) {
		double p0, p1, p2;
		if (abs(points[i].wcs.point.x - points[i + 1].wcs.point.x) <= 0.1) {
			p0 = points[i - 1].diff;
			p1 = points[i].diff;
			p2 = points[i +2].diff;
		}
		...
		if (abs(points[i].wcs.point.x - points[i + 1].wcs.point.x) <= 0.1) {
			zSmoothed[i + 1] = clamp(z, minZ, maxZ);
			i++;
		}
	}
	...
	for (size_t i = 0; i < n; ++i) {
		qDebug() << "origin compen_value :" << smoothedPoints[i].diff << "  " << zSmoothed[i]<<"  "<<i;
...
void laserCompensate::readSensorData(const QString sensorDataPath)
{
	...
	std::vector<SENSOR_DATA_WITH_POS> pOriginLaserData ;
	QString line;
	vecSensorData.clear();
	...
	for (int i = 0; i < pStrLst->size(); i++)
	{
		...
		if (LASER_COMP_TYPE == mMeasureType)
		{
			if (!pOriginLaserData.empty() && abs(tmp[0].toDouble() - pOriginLaserData.back().wcs[0])<0.5) {
				continue;
			}
			...
			//for (int j = AXIS_NUMBER; j < tmp.size()&& j < AXIS_NUMBER + 2; j++)
			//{
			...
			int laser_count = 0;
			tmpSensorValue = 0.0;
			for (int j = AXIS_NUMBER; j < tmp.size()-1 && j < AXIS_NUMBER + 3; j++)
			{
				if (true == tmp.at(j).trimmed().isEmpty())
				{
					break;
			...
			//求均值
			singleLaserData.sensorVal = tmpSensorValue/laser_count;
			//去重并且过滤异常值
			if((pOriginLaserData.empty()||abs(singleLaserData.wcs[0]- pOriginLaserData.back().wcs[0])>0.5)
				&&abs(singleLaserData.sensorVal)<=5.0 && singleLaserData.wcs[5]==90.0)
				pOriginLaserData.push_back(singleLaserData);
		}
	...
	if (LASER_COMP_TYPE == mMeasureType)
	{
		...
		//	}
		//}
		vecSensorData = std::move(pOriginLaserData);
	}
...
double polynomialFit_calculate_thickness(const std::vector<double>& coefficients,double x) {
	double y = 0.0;
...
int laserCompensate::generateProgram(const QString sensorDataPath, const QString originNCPath, bool isPositive)
{
	...
	{
		QMessageBox::warning(NULL, "warning", QString::fromLocal8Bit("未找到进退刀标识"), QMessageBox::Yes);
		return ERROR_SLOEM;
	}
	origin_index = findMinimumKeywordIndex(*pStrLst, { "G54", "G55", "G56", "G57", "G58" });
	if (0 > origin_index || begin_index <= origin_index)
	{
		QMessageBox::warning(NULL, "warning", QString::fromLocal8Bit("未定义工件坐标系"), QMessageBox::Yes);
		return ERROR_SLOEM;
	}
	for (int i = 0; i < begin_index; i++)
	{
		...
		extract_number_from_mpf(line, curPos);
		qDebug() << curPos.data;
	}
	...
	for (int i = begin_index + 1; i < end_index; i++)
	{
		...
		if (LASER_COMP_TYPE == mMeasureType)
		{
			...
			//index = laser_data_comparison(curPos, vecSensorData, 0);
			...
			{
				qDebug() << line;
				qDebug() << curPos.point.x << curPos.point.y << curPos.point.z << curPos.point.a << curPos.point.b << curPos.point.c;
				qDebug() << vecSensorData.at(former_index).wcs[0] << vecSensorData.at(former_index).wcs[1] <<
					vecSensorData.at(former_index).wcs[2] << vecSensorData.at(former_index).wcs[3] << vecSensorData.at(former_index).wcs[4] << vecSensorData.at(former_index).wcs[5];
		...
		else
		{
			if (0 == sameIdex)
			{
				//index = measurement_data_comparison(curPos, vecSensorData, (0 < former_index - 100) ? former_index - 100 : 0);
				//index = measurement_data_comparison(curPos, vecSensorData, (0 < former_index - 1000) ? former_index - 1000 : 0);
			}
			else
			{
		...
		{	
			...
			{
				float avrSensorVal = 0.0;
				for (auto vecIndec : indexLst)
				{
					avrSensorVal += vecSensorData.at(vecIndec).sensorVal;
				}
				avrSensorVal /= indexLst.size();
				tmpPointDiff.diff = coefficient * (avrSensorVal - vecSensorData.at(index).standard_value);
			}
		}
		else {
			return ERROR_SLOEM;
	...
	QString externName = (mMeasureType == LASER_COMP_TYPE) ? "_JG." :"_CS.";
	// 新文件的路径（相同文件夹 + 原文件名 + 当前时间）
	QString newFilePath = directory + "/" + fileName.left(fileName.lastIndexOf('.'))
		+ externName + extension;
	calculateComVal(vecPointDiff, *pStrLst, newFilePath);
